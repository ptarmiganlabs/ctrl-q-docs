/**
 * Fetch latest Ctrl-Q release tag and write to docs/.vitepress/version.js
 * This runs before dev and build. If the request fails, we keep the last
...
async function writeVersionFile(version) {
  const content = `// Auto-generated by scripts/fetch-ctrl-q-version.mjs - do not edit\nexport const version = "${version}";\n`;
...
async function main() {
  try {
    const headers = { "User-Agent": "ctrl-q-docs-build-script" };
    ...
    // Normalize: keep from first 'v' onwards (e.g., 'v4.5.0' -> 'v4.5.0')
    const vIndex = rawTag.indexOf("v");
    const tag = vIndex >= 0 ? rawTag.slice(vIndex) : rawTag;

    await writeVersionFile(tag);
    console.log(`[ctrl-q-docs] Latest ${REPO} version: ${tag}`);
  } catch (err) {
    console.warn(
      `[ctrl-q-docs] Warning: Could not fetch latest release tag: ${err.message}`
    );
    // If the file already exists, keep it. Otherwise write a safe default.
    try {
      const existing = await fs.readFile(outFile, "utf8");
      if (existing) {
        console.log("[ctrl-q-docs] Using existing generated version.js");
        return;
      }
    } catch {}
    const fallback = "v0.0.0";
    await writeVersionFile(fallback);
    console.log(`[ctrl-q-docs] Wrote fallback version ${fallback}`);
  }
}

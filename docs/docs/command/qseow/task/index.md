...
- [List tasks as network graph](#list-tasks-as-network-graph)
  - [Syntax](#syntax-1)
  - [Example](#example)
  - [Filtering which tasks are visualised](#filtering-which-tasks-are-visualised)
  - [Startup checks: circular chains and duplicate triggers](#startup-checks-circular-chains-and-duplicate-triggers)
...
The table view can be enhanced with colours (see [this page](/docs/getting-started/colors-formatting/)) and task details.
...
Options:
  ...
  --auth-jwt <jwt>              JSON Web Token (JWT) to use for authentication with Qlik Sense server
  --task-type <type...>         type of tasks to include (choices: "reload", "ext-program")
  --task-id <ids...>            task(s) in task chains to include in the network graph.
  --task-tag <tags...>          task tag(s) to include in the network graph.
  --app-id <ids...>             app(s) associated with the tasks that should be included in the network graph.
  --app-tag <tags...>           app tag(s) associated with the tasks that should be included in the network graph.
...
### Filtering which tasks are visualised

By default `task-vis` shows all reload and external program tasks in the Sense environment.  
For larger environments this can produce a network graph that is hard to read. Several filter options can be used to narrow the visualisation:

| Option | Effect |
| ------ | ------ |
| `--task-type <type...>` | Limit by task type. Choices: `reload`, `ext-program`. |
| `--task-id <ids...>` | Include only the specified task(s) and the task chains they belong to. |
| `--task-tag <tags...>` | Include tasks that have any of the specified tags, plus the task chains they belong to. |
| `--app-id <ids...>` | Include tasks associated with the specified app(s). |
| `--app-tag <tags...>` | Include tasks associated with apps that have any of the specified tags. |

Filters are **additive** — a task is included in the network graph if it matches any of the specified filters. The matching root tasks are de-duplicated, and the full task chain for each match is then built and visualised.

If none of `--task-id`, `--task-tag`, `--app-id` or `--app-tag` are specified, all tasks in the task model are visualised.

Examples:

Show only the network around two specific reload tasks:
...
.\ctrl-q.exe qseow task-vis `
  ...
  --auth-user-dir LAB `
  --auth-user-id goran `
  --task-id 11111111-2222-3333-4444-555555555555 22222222-3333-4444-5555-666666666666
```

Show all tasks tagged `Finance` or `Sales`:
...
.\ctrl-q.exe qseow task-vis `
  ...
  --auth-user-dir LAB `
  --auth-user-id goran `
  --task-tag Finance Sales
```

### Startup checks: circular chains and duplicate triggers

When `task-vis` starts, Ctrl-Q analyses the task model before launching the visualisation server and reports two kinds of findings to the console:

- **Circular task chains** — chains where a task eventually triggers itself, directly or via other tasks. These are usually unintentional and can lead to runaway reload loops.
- **Duplicate task triggers** — cases where the same upstream task triggers the same downstream task more than once via identical triggers.

Any duplicate identical triggers are also annotated in the network graph: the edge between the two tasks is labelled with the number of identical triggers, making them easy to spot visually.
...
The legend explains both the colors used for triggers and task statuses, and the shapes used for the different node types: triangle for schedule trigger, hexagon for composite trigger, ellipse for external program task, and rounded rectangle for reload task.
...
Double-clicking any node in the network graph opens a modal with the node's basic information: ID, name and type. This is a quick way to confirm exactly which task or trigger a given node represents without having to hover for a tooltip.